...
class NewsAPINewsService(BaseSearchService):
    ...
    def get(self, req, lookup):
        ...
        exclude_fields = (
            self.mandatory_exclude_fields.union(set(orig_request_params.get("exclude_fields").split(",")))
    ...
    def prefill_search_products(self, search):
        ...
        if search.args.get("products"):
            search.products = list(
                get_resource_service("products").get(
                    req=None,
                    lookup={
                        "is_enabled": True,
                        "companies": ObjectId(search.company["_id"]),
                        "_id": {"$in": [ObjectId(pid) for pid in search.args["products"].split(",")]},
                    },
                )
            )
        else:
    ...
    def prefill_search_page(self, search):
        ...
        if isinstance(search.args["sort"], str):
            if search.args["sort"] == "versioncreated:desc":
                ...
                raise BadParameterValueError("Unknown sort option ({name})".format(name=search.args["sort"]))
    ...
    def validate_include_exclude_fields(self, search):
        ...
        :param newsroom.search.SearchQuery search: The search query instance
        """
        if search.args.get("include_fields") and not all(
            p in [c for c in list(self.allowed_include_fields)] for p in search.args["include_fields"].split(",")
        ):
            raise UnexpectedParameterError("Include fields contains a non-allowed value")
        elif search.args.get("exclude_fields") and not all(
            p in [c for c in list(self.allowed_exclude_fields)] for p in search.args["exclude_fields"].split(",")
        ):
            raise UnexpectedParameterError("Exclude fields contains a non-allowed value")
