    ...
    with template_locale(timezone=tz):
        ...
        if is_tbc_item:
            if schedule_type == ScheduleType.MULTI_DAY:
                return str(lazy_gettext("Date: {start} to {end} ({tz}) (Time to be confirmed)")).format(
                    start=formatted_start, end=formatted_end, tz=tz
                )

            else:
                return str(lazy_gettext("Date: {start} ({tz}) (Time to be confirmed)")).format(
                    start=formatted_start, tz=tz
                )

        elif schedule_type in (ScheduleType.ALL_DAY, ScheduleType.NO_DURATION):
            return str(lazy_gettext("Date: {start} ({tz})")).format(start=formatted_start, tz=tz)

        elif schedule_type == ScheduleType.MULTI_DAY:
            return str(lazy_gettext("Date: {start} to {end} ({tz})")).format(
                start=formatted_start, end=formatted_end, tz=tz
            )
        elif schedule_type == ScheduleType.REGULAR:
            return str(lazy_gettext("Time: {start_time} to {end_time} on Date: {date} ({tz})")).format(
            ...
            # Default
            return str(lazy_gettext("Date: {start_time} {start_date} to {end_time} {end_date} ({tz})")).format(
                start_time=notification_time(start),
