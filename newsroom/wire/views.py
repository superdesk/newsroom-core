...
async def get_view_data() -> Dict:
    ...
    user_folders = await get_user_folders(user_dict, "wire") if user else []
    company_folders = await get_company_folders(company_dict, "wire") if company_dict else []
...
async def download():
    ...
    get_resource_service("history").create_history_record(
        items, "download", user.to_dict(), request.args.get("type", "wire")
    )
...
async def share():
    ...
    get_resource_service("history").create_history_record(
        items, "share", current_user_dict, request.args.get("type", "wire")
    )
    return jsonify(), 201
...
async def copy(_id):
    ...
    get_resource_service("history").create_history_record(
        [item], "copy", get_user_from_request(None).to_dict(), request.args.get("type", "wire")
    )
...
async def item(_id):
    ...
    if "print" in request.args:
        ...
        get_resource_service("history").create_history_record(
            [item], "print", get_user_from_request(None).to_dict(), request.args.get("type", "wire")
        )
