...
async def get_home_data():
    ...
    cards = await (await CardsResourceService().find({"dashboard": "newsroom"}, max_results=500)).to_list_raw()
    topics = await get_user_topics_async(user)
    ui_config_service = UiConfigResourceService()
...
async def get_card_items() -> Response:
    company = get_company_from_request(None)
    cards = await (await CardsResourceService().find({"dashboard": "newsroom"}, max_results=500)).to_list()
