...
from newsroom.types import AgendaItem
...
def gen_plans(item: Dict[str, Any], plan_date: int, coverage_dates: Optional[List[int]] = None) -> AgendaItem:
    ...
    item.update(
        {
            ...
            "coverages": [
                {
                    "scheduled": date_to_str(now_minus_days(coverage_date)),
                ...
                for coverage_date in coverage_dates or []
            ],
        }
    )

    return AgendaItem.from_dict(item)


def gen_event(item: Dict[str, Any], event_end_date: int, plan_ids: Optional[List[str]] = None) -> AgendaItem:
    item.update(
        {
            "item_type": "event",
            "state": "scheduled",
            ...
            "planning_items": [
                {
                    "_id": plan_id,
                    "guid": plan_id,
                    "planning_date": now_minus_days(event_end_date) - timedelta(hours=1),
                    "state": "scheduled",
                }
...
async def test_get_expired_chain_ids(app):
    ...
    assert await _get_expired_chain_ids(plan1, expiry_datetime) == set()
    assert await _get_expired_chain_ids(plan2, expiry_datetime) == {"plan2"}
    assert await _get_expired_chain_ids(event1, expiry_datetime) == {"event1"}
    assert await _get_expired_chain_ids(event2, expiry_datetime) == {
        "event2",
        "plan3",
        "plan4",
    }
    assert await _get_expired_chain_ids(event3, expiry_datetime) == set()
    assert await _get_expired_chain_ids(event4, expiry_datetime) == set()
...
async def test_remove_expired_agenda(runner, app):
    ...
    item_ids = [item["_id"] for item in await get_all("agenda")]
    for item_id in ids_to_keep + ids_to_purge:
        assert item_id in item_ids, item_ids
