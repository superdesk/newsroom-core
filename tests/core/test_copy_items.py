...
from ..utils import load_fixture, load_json_fixture, login
...
async def test_copy_agenda_with_restricted_coverage_details(client, app, restrict_user):
    item = load_json_fixture("agenda_copy_fixture.json")
    await create_entries_for("agenda", [item])
    item_id = item["_id"]

    await login(client, restrict_user)
    resp = await client.post(f"/wire/{item_id}/copy?type=agenda")
    data = json.loads(await resp.get_data())
    assert resp.status_code == 200
