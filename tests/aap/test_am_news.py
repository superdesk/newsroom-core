...
def test_item_copy(client, app):
    resp = client.post("/am_news/{}/copy".format(items[0]["_id"]), content_type="application/json")
    assert resp.status_code == 200
...
def test_search_created_to(client):
    ...
    resp = client.get(
        "/am_news/search?created_to=%s&timezone_offset=%s"
...
def test_download(client, app):
    ...
    assert (len(wire_formats) * len(item_ids)) == count
