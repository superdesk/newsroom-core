...
def test_item_copy(client, app):
    resp = client.post("/factcheck/{}/copy".format(items[0]["_id"]), content_type="application/json")
    assert resp.status_code == 200
...
def test_search_created_to(client):
    ...
    resp = client.get(
        "/factcheck/search?created_to=%s&timezone_offset=%s"
